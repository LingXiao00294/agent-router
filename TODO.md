...
- [x] 将 Provider 列表行改为与虚拟模型页面一致的卡片布局。
- [x] 卡片展示 Provider 名称、类型、密钥状态和简化后的 Base URL。
- [x] 卡片小字展示实际模型数量与模型名。
- [x] 模型较多时显示有限数量并追加“还有 N 个”。
- [x] 卡片提供“添加模型”“设置”“删除”操作。
- [x] 无实际模型时显示明确的空状态。

### Provider 设置弹窗

- [x] 保留现有 Provider 基础设置字段。
- [x] Provider 设置弹窗不直接承担实际模型表单。
- [x] 保留 API key 脱敏值、留空保留和新 Provider 必填规则。
- [x] 保留限流、队列、超时和熔断覆盖配置。

### 实际模型弹窗

- [x] 点击“添加模型”打开空白实际模型弹窗。
- [x] 点击卡片中的实际模型名打开编辑弹窗。
- [x] 弹窗显示统一展示名 `<provider>/<model>`。
- [x] 新建实际模型时允许填写模型名和四类价格。
- [x] 编辑已有实际模型时模型名只读，只允许修改四类价格。
- [x] 模型改名严格使用“新建模型、切换引用、删除旧模型”流程，不在编辑弹窗内伪装成原地重命名。
- [x] 校验模型名非空且在同一 Provider 下唯一。
- [x] 校验价格为空或非负数。
- [x] 已被引用的模型不允许直接重命名。
- [x] 删除被引用模型时展示引用它的虚拟模型并保持数据不变。
- [x] 删除未被引用模型前显示确认弹窗。

### 虚拟模型页面

- [x] 在 PR 2 已完成的功能基础上打磨引用行布局，不改变数据契约和校验规则。
- [x] 优化 `<provider>/<model>` 长标签、分组选项和空状态的可读性。
- [x] Provider 或模型数量较多时保证选择器尺寸和滚动体验可用。
- [x] 保留现有拖动排序动画和交互。

### 交互与可访问性

- [x] 弹窗支持 Escape 关闭和遮罩关闭。
- [x] 删除、引用冲突和保存结果使用现有确认框与 Toast。
- [x] 键盘焦点进入弹窗并在关闭后合理恢复。
- [x] 小屏布局下卡片、模型名称和操作按钮保持可用。
- [x] 长 Provider 名和长模型名不破坏布局。

### 测试与验证

- [x] 覆盖 Provider 卡片摘要生成逻辑。
- [x] 覆盖添加、编辑、删除实际模型的 store 行为。
- [x] 覆盖引用冲突时的数据保持行为。
- [x] 复用 PR 2 的组合选择、去重和 Pin 功能测试，不在本 PR 重复建立第二套状态逻辑。
- [x] 运行 Dashboard TypeScript 检查和生产构建。
- [x] 对 Provider、虚拟模型和调用详情页面进行人工视觉检查。

### 文档

- [x] 更新 README 的 Dashboard 配置流程。
- [x] 更新 `docs/design.md` 的 Dashboard 信息架构和删除保护交互。
- [x] 如界面变化影响现有截图或示例，同步更新对应内容。

### 验收标准

- [x] Provider 页面是 Dashboard 内管理实际模型的唯一入口；用户仍可直接编辑作为配置真源的 `config.toml`。
- [x] Provider 卡片无需打开弹窗即可看见已配置模型摘要。
- [x] 虚拟模型页面只能从已有实际模型目录中选择。
- [x] 页面中实际模型统一展示为 `<provider>/<model>`。
- [x] 用户无法通过 Dashboard 产生重复引用或悬空引用。
- [x] Dashboard 构建通过，并完成主要页面人工视觉检查。
...
- [x] 每个 PR 从最新主分支创建独立语义化分支。
- [x] 开始修改前检查当前目录、分支和工作区状态。
- [x] 不在 `main` 或 `master` 上直接开发。
- [x] 每个 PR 只包含自身范围内的代码、测试和文档。
- [x] 不做无关格式化、重构、依赖升级或锁文件更新。
- [x] 不在本地把功能分支直接合并到主分支。
- [x] 未经用户明确确认不执行 `git push`。
...
- [x] Provider、实际模型、虚拟模型的职责边界与配置格式一致。
- [x] `<provider>/<model>` 在 Dashboard、Metrics 和调用详情中统一展示。
- [x] 实际模型身份在内部、API 和数据库中始终保留独立 Provider 与模型字段。
- [x] 历史调用保存价格快照，调价不会改变旧记录的解释结果。
- [x] Provider/模型删除保护同时由后端和 Dashboard 执行。
- [x] 三个 PR 均包含与行为变更匹配的测试和文档。
- [x] README、设计文档、示例配置和 Dashboard 行为不存在相互矛盾的描述。
