<template>
  <ConfigSection title="虚拟模型">
    ...
    <ModelCard
      ...
      :pin-enabled="configStore.routerConfig.mode === 'sticky'"
      :name-error="configStore.fieldError(`models[${idx}].name`)"
      :refs-error="configStore.fieldError(`models[${idx}].refs`)"
      :pinned-error="configStore.fieldError(`models[${idx}].pinned`)"
      ...
      @pin-ref="(ri) => { configStore.pinRef(m, ri); touch(`models[${idx}].pinned`) }"
